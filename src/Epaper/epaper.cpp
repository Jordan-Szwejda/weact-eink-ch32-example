...
The most code was taken from
https://github.com/WeActStudio/WeActStudio.EpaperModule/tree/master

and adapted to ch32v003.

It uses my private GPIO implementation that will be published soon.
---------------------------------------*/
...
void epd_dc_set()
{
  GPIO::digitalWrite(PIN_DC, true);
  //GPIO_SetBits(GPIOB, GPIO_Pin_14);
}

void epd_dc_reset()
{
  GPIO::digitalWrite(PIN_DC, false);
...
uint8_t epd_is_busy()
{
  return GPIO::digitalRead(PIN_BUSY);
  //return GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_15) == Bit_RESET ? 0 : 1;
}

static void initSpiPin(PinId pinId, GPIOMode_TypeDef mode)
{
    GPIO_InitTypeDef GPIO_InitStructure = {0};
    GPIO::PinInfo gpioInfo = GPIO::getGpioInfo(pinId);
    GPIO_InitStructure.GPIO_Pin = gpioInfo.gpioNumber;
    GPIO_InitStructure.GPIO_Mode = mode;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(gpioInfo.gpioPort, &GPIO_InitStructure);
}

void epd_io_init(void)
{
  GPIO::pinMode(PIN_RESET, GPIO_Mode_Out_PP);
  GPIO::pinMode(PIN_DC, GPIO_Mode_Out_PP);
  GPIO::pinMode(PIN_CS, GPIO_Mode_Out_PP);
  GPIO::pinMode(PIN_BUSY, GPIO_Mode_IPU);
...
//  GPIO_InitTypeDef GPIO_InitStructure = {0};
  ...
  RCC_APB2PeriphClockCmd(GPIO::getGpioInfo(PIN_SCK).perpipherialClock | 
                        //GPIO::getGpioInfo(PIN_MISO).perpipherialClock | 
                        GPIO::getGpioInfo(PIN_MOSI).perpipherialClock | RCC_APB2Periph_SPI1,ENABLE);


initSpiPin(PIN_SCK, GPIO_Mode_AF_PP);
//initSpiPin(PIN_MISO, GPIO_Mode_IN_FLOATING);
initSpiPin(PIN_MOSI, GPIO_Mode_AF_PP);                        
...
epd_write_data(0x01); // defines address x/y increment-decrement - here y-decrement, x-increment (adres incremented in X direction)
...
uint8_t epd_init_partial(void)
{
  ...
  epd_write_reg(0x32);
  epd_cs_reset();
  for (size_t j = 0; j < sizeof(lut_partial); j++)
  {
...
void epd_write_pattern(uint8_t *_image, uint8_t _widthBytes, uint8_t _height)
{
  uint16_t Width, Height, xPos, yPos;
  uint16_t k = 0;
  Height = EPD_H;
  Width = EPD_W_BUFF_SIZE;
  ...
  for (yPos = 0; yPos < Height; yPos++)
  {
    ...
    {
      uint16_t realY = (yPos % _height);
      uint16_t offsetY = realY * _widthBytes;
      uint16_t realX = (xPos % _widthBytes);
      _epd_write_data(_image[realX+offsetY]);
    }
  }
  _epd_write_data_over();
  epd_cs_set();
...
void epd_write_to_ram_blackWhite(uint8_t *_image, uint16_t bytes)
{
  epd_write_reg(0x24);
  epd_writedata(_image, bytes);
}

void epd_write_to_ram_red(uint8_t *_image, uint16_t bytes)
{
  epd_write_reg(0x26);
  epd_writedata(_image, bytes);
}
